...
实验是已发布资源 Revision 的组合与运行容器，不是资源编辑器。实验详情只保留“实验概览、实验结果、参与 Agent、模型与运行”四个一级页面；地图和 Brain 的创建、编辑、校验、发布均位于全局资源工作区。实验概览只能选择精确的地图 Revision 与 Brain Revision，不能创建地图覆盖层、复制 Brain 文本或打开实验私有流程编排器。

完整交互与 API 边界见 [实验资源组合 UX 与交互边界](experiment-resource-composition-ux.md)。
