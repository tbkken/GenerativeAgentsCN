...
画布节点为声明式数据，不执行浏览器输入的任意源码。Script 节点只能选择服务端注册的确定性 Function；LLM 节点引用当前实验 Prompt；If / Else、Switch、Loop、Parallel / Join、Read State、Write State、Subflow、Start、End 由统一 Schema 校验。
...
- Alembic head 为 `0008_prompt_workflows`。
...
发布校验要求五条流程齐全、29 个系统 Prompt 各放置一次、Prompt 引用存在、Script Function 已注册、端口类型兼容且所有节点从 Start 可达并能到达 End。发布后的 workflow rows 与 ExperimentDefinition 一起只读。
