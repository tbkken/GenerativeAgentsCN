...
| --- | --- | --- | --- |
| 运行日志 log | `generative_agents_next/results/checkpoints/<name>/<name>.log` | 文本日志、模型调用摘要、错误信息。 | 运行是否稳定，是否出现限流、JSON 解析失败或模型异常。 |
| 断点 checkpoint | `generative_agents_next/results/checkpoints/<name>/simulate-*.json` | 每一步的 `time`、`step`、`agents`、坐标、行动、日程、记忆摘要。 | 某一刻角色在哪里、做什么、计划是什么。 |
| 对话记录 conversation | `generative_agents_next/results/checkpoints/<name>/conversation.json` | 时间键下的“说话者 -> 听话者 @ 地点”和多轮原话。 | 谁把信息告诉了谁，事实是否保真。 |
| 时间线 simulation | `generative_agents_next/results/compressed/<name>/simulation.md` | 基础人设、活动记录、对话记录的 Markdown 汇编。 | 人工快速定位证据片段。 |
| 移动回放 movement | `generative_agents_next/results/compressed/<name>/movement.json` | `start_datetime`、`stride`、`persona_init_pos`、`all_movement`。 | 角色是否到达地点、聚集持续多久。 |
...
```text
generative_agents_next/results/evaluations/<experiment_name>/metrics.json
generative_agents_next/results/evaluations/<experiment_name>/report.md
```
...
社会仿真升级不能只跑一次。第 36 章采用三次同配置运行，先看传播、承诺、到场和目标完成率是否稳定。为了避免把批量统计写成空话，`generative_agents_next/analyze_experiment.py` 增加了 `--batch-names` 汇总入口。
...
+    for name in names:
+        metrics = load_json("results/evaluations/<name>/metrics.json", {})
+        rows.append({
+            "mentions": metrics["diffusion"]["mention_count"],
+            "arrived": metrics["attendance"]["arrived_count"],
+            "goal_completion_rate": metrics["goal_progress"]["goal_completion_rate"]
+        })
+    write("batch_summary.json", summary)
```

三次运行使用相同角色、相同时间和相同事件关键词，只改变实验名。
