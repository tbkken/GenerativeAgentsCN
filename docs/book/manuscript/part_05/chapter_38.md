...
1. 生成式智能体 Generative Agents 前沿升级应该遵守什么原则？
2. 哪些升级应该先做，哪些应该后做？
...
*图 38-1：生成式智能体 Generative Agents 前沿升级路线图。路线图按风险和验证难度推进，先增强观测和评价，再逐步改记忆、反思、规划和协作。*

![图 38-2：前沿升级路线的项目落地墙](../../assets/chapter_38/ch38_upgrade_roadmap_wall.png)

*图 38-2：前沿升级路线的项目落地墙。图片把观测评价 evaluation、记忆 memory、反思 reflexion、目标 goal、协作 team 和模型路由 routing 锚定到当前源码、提示词 prompt、断点 checkpoint 与压缩结果 compressed result。*

## 38.2 路线图的基本原则

本章路线图遵守五个原则。第一，先观测，后改行为。如果没有指标和报告，改动后很难判断是否真的变好。第二，先低风险，后高风险。例如增加统计脚本风险低。修改 `Agent.make_plan()` 风险高。第三，先局部增强，后系统重构。例如增加 `relationship` 记忆类型，比重写整个记忆系统 memory system 更适合第一步。第四，每个阶段都要有实验验证。不能只说“架构更先进”。第五，保留生成式智能体 Generative Agents 的核心思想。升级不是把项目改成完全不同的工作流框架。小镇仍然应该保留：
...
```mermaid
flowchart TD
    Observe["阶段一：观测与评价"] --> Memory["阶段二：记忆系统增强"]
    Memory --> Reflect["阶段三：反思学习与技能库"]
    Reflect --> Goal["阶段四：目标驱动规划"]
    Goal --> Team["阶段五：组织化协作"]
    Team --> Routing["阶段六：模型路由与成本优化"]
    Observe -->|支撑| Goal
    Observe -->|支撑| Team
    Observe -->|支撑| Routing
```
...
第一阶段不改智能体 agent 行为。只增强实验观测能力。目标是：
...
1. 扩展记忆类型 memory type。
2. 增加 `relationship` 记忆。
3. 增加关系更新提示词 prompt。
4. 增加记忆来源和证据 evidence 字段。
5. 增加简单冲突检测。
...
1. 增加行动结果 action outcome。
2. 增加自我评价提示词 self-evaluation 提示词 prompt。
3. 增加 lesson 记忆。
4. 增加技能 skill 记忆。
5. 在对话和计划前检索相关技能 skill。
...
1. 增加目标 Goal 数据结构。
2. 给角色增加主动目标 active goals。
3. 增加目标分解 goal decomposition。
4. 增加 候选行动生成 candidate action generation。
5. 增加目标进度评估 goal progress evaluation。
6. 将目标 goal 结果写回记忆。
...
这一步风险高。因为它会改变智能体 agent 行动选择。因此建议只对明确事件启用。例如：
...
- 目标完成率 goal_completion_rate。
- 知情角色数 unique_informed_agents。
- accepted_count。
- attendance_count。
- naturalness_score。
...
- 目标进度准确率 goal_progress_accuracy。
...
- 多智能体归因可追踪性 multi_agent_credit_traceability。
...
前面几个阶段会增加大语言模型 LLM 调用。如果全部使用强模型，成本会很高。如果全部使用小模型，结构化输出和反思质量可能不稳。因此需要模型路由。当前项目配置是：
...
可以先不重构模型提供方 provider。最小方案是扩展配置：
...
```mermaid
flowchart TD
    Call["一次模型调用 completion"] --> Type{"调用类型"}
    Type -->|日常对话| Daily["日常模型 daily model"]
    Type -->|记忆反思| Reflection["反思模型反思模型 reflection model"]
    Type -->|目标规划| Planning["规划模型规划模型 planning model"]
    Daily --> Summary["记录成功率、耗时、成本"]
    Reflection --> Summary
    Planning --> Summary
    Summary --> Metrics["写入评价指标 metrics"]
    Metrics --> Decision["判断路由是否值得保留"]
```
...
有些事听起来高级，但不适合一开始做。第一，不建议一开始重写整个智能体 Agent 架构。会破坏读者对原论文结构的理解。第二，不建议一开始引入复杂工作流框架。生成式智能体 Generative Agents 的价值在于小镇社会仿真，不是企业流程编排。第三，不建议一开始追求大规模智能体 agent 数量。如果 5 个角色都评价不清，25 个或 100 个只会更乱。第四，不建议一开始做跨实验长期记忆。这会显著影响可复现性。第五，不建议只换强模型。强模型可能提高输出质量，但不能替代记忆、评价和环境约束。路线图要务实。先让系统可观测，再让能力增强。

## 38.17 与原论文思想的一致性

这条路线虽然引入很多前沿思想，但仍然围绕生成式智能体 Generative Agents 原始架构。对应关系如下：

| 原论文模块 | 当前项目 | 升级方向 |
| --- | --- | --- |
| 记忆流 Memory Stream | Associate / 概念节点 Concept | 记忆治理、关系记忆、冲突检测 |
| 检索 Retrieval | AssociateRetriever | 场景化检索、来源追踪 |
| 反思 Reflection | Agent.reflect() | 失败复盘、经验 lesson、技能 skill |
| 规划 Planning | 日程 Schedule / make_schedule | 目标 Goal、候选行动、进度评估 |
| Reacting | _reaction() | 目标感知反应、冲突处理 |
| 对话 Dialogue | _chat_with() / generate_chat | 协作对话协议、关系驱动对话 |
| Smallville | 世界地图 Maze / Phaser replay | 批量实验、统计指标 |
| 评价 Evaluation | 时间线 simulation / 移动回放 movement | 指标 metrics、报告 report、基线、成本 |
...
```mermaid
flowchart TD
    TwoWeeks["两周：观测与评价增强"] --> OneMonth["一个月：加入关系记忆"]
    OneMonth --> ThreeMonths["两到三个月：加入失败复盘和技能库"]
    ThreeMonths --> Research["研究项目：目标规划与组织化协作"]
    Research --> Maintain["长期维护：模型路由与成本优化"]
    TwoWeeks --> Baseline["始终保留默认系统基线"]
    Research --> Baseline
    Maintain --> Baseline
```
