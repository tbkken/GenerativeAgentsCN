...
Generative Agents 的第一块基石是 Memory Stream。

没有记忆，智能体只能对当前 prompt 做反应。
...
但 Memory Stream 只是起点。

到了 2026 年，长期智能体系统已经不能只满足于“把事情记下来，再用向量检索找回来”。

本章要回答六个问题：

1. Generative Agents 的 Memory Stream 解决了什么问题？
2. GenerativeAgentsCN 当前如何实现记忆？
3. 当前实现有什么长期运行瓶颈？
4. MemGPT 和 Mem0 给我们什么启发？
5. 如何把这些启发落到 GenerativeAgentsCN 的代码结构中？
6. 如何评价记忆升级是否真的有效？

[图 29-1：从 Memory Stream 到记忆治理层的演进]
...
其中 `event` 又由 subject、predicate、object、address 和 describe 构成。
...
`AssociateRetriever` 的检索逻辑非常接近论文思想。
...
有时候和当前问题相关的旧事件会被重新激活。

当前实现已经保留了 Generative Agents 的核心判断。
...
所有记忆都进入同一个 `Associate` 结构。

短期工作记忆、长期情景记忆、语义记忆、关系记忆没有区分。

第三，记忆没有合并机制。

如果角色每天都去咖啡馆，系统可能反复保存类似事件。

久而久之，检索噪声会变大。

第四，记忆冲突没有显式处理。

例如派对时间被说成 17:00，又被说成 19:00。

系统没有专门机制判断哪个更可信。

第五，关系记忆不够稳定。

`Associate.get_relation()` 会基于某个 node 的描述检索 events 和 thoughts。

这很灵活，但关系状态不是显式对象。

例如“汤姆不信任山姆”这种长期关系，最好有稳定结构。

第六，跨实验记忆没有清晰策略。

当前项目适合单次仿真。

如果希望角色跨多个实验成长，就需要更明确的长期记忆库。
...
这和 Generative Agents 的 Memory Stream 有明显区别。
...
这对 GenerativeAgentsCN 的启发非常直接。

当前 `Associate.retrieve_focus()` 可以取回相关记忆。
...
Mem0 面向生产级 AI agent 的长期记忆。

它强调的不是小镇仿真，而是更通用的 agent 场景：
...
对 GenerativeAgentsCN 来说，Mem0 的启发是：
...
另外，`retrieve_events()`、`retrieve_thoughts()`、`retrieve_chats()` 可以保留。
...
例如汤姆不喜欢山姆，这可能写在 `agent.json` 的 learned 或 currently 中，也可能进入 thought。

但如果希望长期关系稳定，最好引入结构化关系记忆。
...
这个 summary 可以用于后续计划和反思。

原始节点仍然保留一段时间，防止摘要损失细节。
...
如果要把 GenerativeAgentsCN 变成长期角色平台，可以引入跨实验记忆。
...
## 29.16 升级方向六：记忆来源与置信度

为了降低记忆幻觉，建议每条高级记忆增加来源和置信度。
...
当前 `retrieve_focus()` 主要按 focus 文本检索 event 和 thought。
...
如果读者想真正动手升级，不建议一开始做完整长期记忆系统。

可以按三步走。

第一步，增加 `relationship` 记忆类型。

理由：

关系记忆和小镇社会行为直接相关，最容易观察价值。

第二步，增加 relationship update prompt。

每次关键对话后，根据对话结果更新双方关系摘要。

第三步，在生成对话时显式检索 relationship。
...
长期 agent 需要的不是无限记忆，而是可治理记忆。

## 29.21 本章小结

本章讨论了从 Memory Stream 到可管理长期记忆的升级路线：

1. Generative Agents 的 Memory Stream 让智能体拥有行为连续性。
2. GenerativeAgentsCN 当前通过 `Associate`、`Concept` 和 LlamaIndex 实现 event/chat/thought 三类记忆。
3. 当前检索结合 recency、relevance 和 importance，保留了论文核心思想。
4. 长期运行会遇到类型不足、无分层、重复记忆、冲突记忆、关系不稳定和跨实验记忆缺失等问题。
5. MemGPT 启发我们把上下文窗口和长期记忆视为主存与外存，需要主动记忆管理。
6. Mem0 启发我们关注生产级、跨会话、个性化长期记忆。
7. GenerativeAgentsCN 可以先扩展 relationship、goal、summary、skill 等记忆类型。
8. 关系记忆、记忆合并、冲突检测、跨实验记忆和来源置信度是可落地升级方向。
9. 记忆升级必须用引用准确率、来源追踪率、关系一致性、冗余率和冲突处理质量评价。
10. 记忆越强，越要重视幻觉、偏见、隐私和可复现性。
