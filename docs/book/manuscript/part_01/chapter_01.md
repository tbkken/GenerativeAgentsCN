...
2023 年，Joon Sung Park、Joseph C. O'Brien、Carrie J. Cai、Meredith Ringel Morris、Percy Liang 和 Michael S. Bernstein 发表了论文 *Generative Agents: Interactive Simulacra of Human Behavior*，成为生成式智能体方向绕不开的起点之一。

![图 1-1：生成式智能体 Generative Agents 的诞生：论文源头、人机交互 HCI 视角与虚拟小镇架构](../../assets/chapter_01/ch01_generative_agents_origin_workbench.png)

*图 1-1：生成式智能体 Generative Agents 的诞生*

## 1.1 作者简介

论文的六位作者来自 Stanford、Google Research 和 Google DeepMind 相关研究环境，研究背景横跨人机交互、大语言模型、社会计算、基础模型和智能体系统。这组背景决定了论文的系统性质：它不是普通 LLM 应用，而是一篇系统味很重的 HCI 论文。
...
该研究最初于 2023 年 4 月在 arXiv 释出预印本并引爆全网，随后于同年 10 月在 ACM UIST 2023（ACM 用户界面软件与技术大会）上正式宣读并收录。
...
Generative Agents 要解决的不是单点模型能力，而是一组互相牵连的系统问题，这些问题都能在不同研究领域中找到解决方案。
...
*图 1-2：生成式智能体 Generative Agents 的领域交叉位置。论文的关键不是把某一个方向做到极致，而是把这些问题合成一个可运行的系统。*
...
*图 1-3：普通大语言模型 LLM 应用的基本结构。*

LLM 应用主要围绕一次输入和一次回答组织，历史上下文只是辅助材料。
...
*图 1-4：生成式智能体的基本结构。系统围绕角色在世界中的持续行为组织，语言生成只是其中一个环节。* 

可以看到，因为生成式智能体的计划、记忆、反思和对话都用自然语言表达，所以自然语言能力仍然很重要。但自然语言表达肯定不是生成式智能体的终点，语言在这里是智能体理解世界、表达经验和生成行动的中间媒介。

## 1.5 可信行为的边界

在人机交互（HCI）和人工智能领域，“可信行为”（Believable Behavior） 并不是指智能体说的话百分之百正确，不是说智能体真的有意识，而是指“智能体的行为符合人类对现实生活中一个正常人的认知和预期，让观察者相信它是一个真正具有意识、情感和社交能力的生命体”。

简单来说，就是 AI 表现得“像个真人”，不会让人产生违和感或出戏。论文用 believable simulacra of human behavior 来描述这个目标：做出像人类行为那样可理解、可跟随的行为拟像，而不是证明机器拥有真实主体性。
...
*图 1-5：可信行为的形成路径。角色行为能被身份、记忆、计划和环境共同解释，读者就会觉得它“像一个持续生活的人”。*
...
*图 1-6：不可信行为的断裂路径。只要身份、记忆、场景或计划中的任一环断开，角色就容易从“可信拟像”退化成随机文本生成。*

后面读 Generative Agents 时，也可以用下面这张表来作为判断标准：角色会聊天、会行动、会反思，但这些表现来自代码、提示词、记忆检索和模型生成。它们可以形成行为连续性，却不能被当作真实的人。
...
*图 1-7：脚本 NPC 的规则膨胀路径。事件越开放，脚本越需要覆盖角色、关系、地点和例外情况。*
...
*图 1-8：生成式智能体的局部决策传播。系统不需要预先写死每个人的分支，而是让信息通过记忆、对话和计划扩散。*

## 1.7 强化学习的局限

脚本 NPC 之外，研究智能体如何在环境中行动的常见路线是强化学习。强化学习在很多游戏环境中取得过非常强的结果，尤其是目标明确、奖励函数清楚、动作空间可定义的任务。例如在对抗游戏中，智能体可以围绕胜负优化策略。
...
*图 1-9：强化学习的典型闭环。它围绕状态、动作、奖励和策略更新组织，适合目标明确、奖励可计算的任务。*

Generative Agents 要解决的不是“赢一局游戏”，而是“表现出可信的人类生活”。可信生活没有简单奖励函数。一个角色早上是否应该喝咖啡、是否应该和朋友聊天、是否应该接受派对邀请，很难用一个统一的数值奖励定义。更重要的是，角色行为的合理性高度依赖自然语言背景、社会关系和过去经历。一个人拒绝派对邀请，有时是合理的，因为他正在赶论文；有时是不合理的，因为他刚刚表示非常期待参加。一个角色没有和熟人打招呼，有时是失礼，有时是因为他正在赶时间。这样的判断离不开上下文和常识。
...
*图 1-10：强化学习与生成式智能体的系统分工。可量化、可枚举、可反馈的部分适合强化学习；开放语境下的常识性行为生成，更适合交给大语言模型 LLM 驱动的智能体架构。*

换句话说，Generative Agents 不是用 LLM 替代所有系统设计，而是用 LLM 补上传统规则系统最难处理的一部分：开放语境下的常识性行为生成。

| 对比维度 | 适合强化学习 | Generative Agents 的诉求 | 优缺点评价 |
| --- | --- | --- | --- |
...
*图 1-11：生成式智能体 Generative Agents 架构闭环。观察、记忆、检索、反思、规划、行动和环境变化连在一起后，单次语言生成才变成持续行为。*

| 中文模块 | 架构模块 | 负责的问题 | 对可信行为的作用 |
| --- | --- | --- | --- |
| 沙盒环境 | Sandbox Environment | 提供时间、地点、对象、角色和事件。 | 把语言生成约束在具体世界中。 |
| 观察 | Observation | 让智能体感知附近发生了什么。 | 行为能响应环境，而不是脱离场景空转。 |
| 记忆流 | Memory Stream | 保存观察、行动、对话和反思。 | 维持身份、经历和社会关系的连续性。 |
| 检索 | Retrieval | 从记忆中选出当前最相关内容。 | 避免每次决策都只依赖当前提示词 prompt。 |
| 反思 | Reflection | 从碎片事件中归纳高层想法。 | 让角色形成更稳定的自我理解和他人理解。 |
| 规划 | Planning | 生成日程和下一步行动。 | 让行为有节奏、有目标，而不是随机游走。 |
| 反应 / 行动 | Reacting / Action | 根据计划和现场变化执行或重规划。 | 让角色能处理突发事件和他人行为。 |
| 对话 | Dialogue | 支持角色之间交流信息。 | 让一个人的记忆进入另一个人的记忆，形成社会传播。 |

这套闭环是后面阅读 Generative Agents 源码的主线：每个类、提示词 prompt、检索器和回放文件，都可以放回这张图里理解。

## 1.9 本章小结

本章从不同的角度介绍了 Generative Agents 这个项目，引导大家把它放在正确位置上：它不是聊天机器人，也不是传统 NPC 的脚本升级，而是一套让角色在小镇世界里持续生活、记忆、计划、交流的系统架构。下一章开始进入论文的核心问题：一个智能体要在小镇里表现得像一个持续生活的人，为什么只会聊天远远不够。
