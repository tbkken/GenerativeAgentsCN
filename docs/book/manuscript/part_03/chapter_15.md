...
```mermaid
flowchart TD
    Args["命令行参数 agents / stride / start_time"] --> Skeleton["新仿真配置骨架 config"]
    Runtime["公共运行配置 agent_base"] --> Skeleton
    Maze["地图文件 maze.json"] --> Skeleton
    Skeleton --> Loop{"逐个角色 agents"}
    Loop --> Path["角色配置路径 config_path"]
    Path --> Agents["角色表 config['agents'][角色名]"]
    Agents --> SimConfig["仿真配置 simulation config"]
```
...
```mermaid
flowchart TD
    Base["公共配置 agent_base"] --> Copy["深拷贝 copy.deepcopy()"]
    SeedPath["角色配置路径 config_path"] --> Load["读取角色配置 load_static()"]
    Copy --> MergeSeed["第一次递归合并 update_dict()"]
    Load --> MergeSeed
    MergeSeed --> Check{"角色条目 agent 是否含断点 checkpoint 字段"}
    Check -->|否，新仿真| OverlayNew["叠加角色路径等轻量字段"]
    Check -->|是，恢复运行| OverlayResume["叠加 action / status / schedule / coord 等状态"]
    OverlayNew --> MergeRuntime["第二次递归合并 update_dict()"]
    OverlayResume --> MergeRuntime
    MergeRuntime --> Storage["独立存储目录 storage_root"]
    Storage --> Build["构造智能体 Agent"]
```
...
真实模板如下。它是第 15 章最重要的提示词 prompt，因为它把角色配置 `agent.json` 里的身份字段转换成后续所有模型调用都能读取的人格底座：
...
这个模板很短，但信息密度很高。每个变量都从初始化对象里来：

| 模板变量 | 中文 English | 数据来源 | 对后续行为的影响 |
| --- | --- | --- | --- |
| `${name}` | 角色名 name | `agent.json.name` | 决定事件主体 subject、日志名称和提示词里的说话对象。 |
| `${age}` | 年龄 age | `agent.json.scratch.age` | 给角色身份阶段和语言风格提供背景。 |
| `${innate}` | 先天特质 innate traits | `agent.json.scratch.innate` | 给角色稳定性格底色，例如外向、好奇、谨慎。 |
| `${learned}` | 后天经历 learned traits | `agent.json.scratch.learned` | 给角色职业、社会身份、知识背景和价值判断。 |
...
| 下游提示词 prompt | 如何使用 base_desc | 行为影响 |
| --- | --- | --- |
| 起床时间 wake_up | 把 `base_desc` 和生活习惯 lifestyle 放在一起 | 决定角色几点醒来。 |
| 日程大纲 schedule_init | 把 `base_desc`、生活习惯 lifestyle、起床时间 wake_up 放在一起 | 生成一天的自然语言大纲。 |
| 小时日程 schedule_daily | 把 `base_desc`、初始日程 daily_schedule 和时间模板放在一起 | 生成 24 小时日程。 |
| 日程拆解 schedule_decompose | 把 `base_desc` 和当前计划 plan 放在一起 | 把粗计划拆成分钟级子任务。 |
...
```mermaid
flowchart TD
    Address["空间记忆地址表 Spatial.address"] --> HasSleep{"已有 sleeping 或 睡觉 地址"}
    HasSleep -->|是| Keep["保留原配置"]
    HasSleep -->|否| HasLiving{"是否有居住地址 living_area"}
    HasLiving -->|否| NoAuto["不自动派生睡觉地址"]
    HasLiving -->|是| Add["写入 睡觉 = living_area + 床"]
...
```mermaid
flowchart TD
    Config["完整角色配置 config"] --> HasAction{"是否含当前行动 action"}
    HasAction -->|否，新仿真| ReadTile["按初始坐标 coord 读取地图格子 Tile"]
    ReadTile --> Address["读取游戏对象 game_object 地址"]
    Address --> Create["创建当前行动 Action"]
    HasAction -->|是，断点恢复| Restore["从字典恢复 Action.from_dict()"]
    Restore --> Tiles["按行动地址查找候选地图格子 get_address_tiles()"]
    Tiles --> Pick["随机选择恢复坐标 coord"]
    Create --> Move["移动函数 move(coord, path)"]
    Pick --> Move
    Move --> Coord["写入角色坐标 coord 与移动路径 path"]
```
...
```mermaid
flowchart TD
    Move["移动函数 move()"] --> Event["角色事件 self.get_event()"]
    Event --> UpdateTile{"地图格子 Tile 已有同类事件"}
    UpdateTile -->|是| Replace["更新事件 update_events()"]
    UpdateTile -->|否| Add["追加事件 add_event()"]
    Move --> ObjEvent["对象事件 self.get_event(False)"]
    ObjEvent --> HasObj{"是否存在对象事件"}
    HasObj -->|是| UpdateObj["世界地图 Maze.update_obj()"]
    HasObj -->|否| Done["只保留角色事件"]
    Replace --> Visible["进入后续感知 percept"]
    Add --> Visible
    UpdateObj --> Visible
    Done --> Visible
```
...
flowchart TD
    ...
    Build --> Prompt["基础提示词预览 Base prompt preview"]
    Identity --> Evidence["初始化证据闭环"]
    Scratch --> Evidence
    Spatial --> Evidence
    Runtime --> Evidence
    Action --> Evidence
...
```mermaid
flowchart TD
    Reset["模型重置函数 reset()"] --> Exists{"大语言模型 LLM 连接 _llm 是否已存在"}
    Exists -->|是| Keep["复用现有模型连接"]
    Exists -->|否| Create["按 think_config.llm 创建模型连接"]
    Keep --> Ready["智能体 Agent 可执行补全 completion()"]
    Create --> Ready
```
...
```mermaid
flowchart TD
    Serialize["智能体序列化函数 Agent.to_dict()"] --> Always["固定保存 status / schedule / associate / chats / currently"]
    Always --> Check{"with_action 是否为真"}
    Check -->|是| Action["追加当前行动 action"]
    Check -->|否| Snapshot["生成不含行动的轻量状态"]
    Action --> Checkpoint["断点 checkpoint 中的 agents[角色名]"]
    Snapshot --> Checkpoint
    Checkpoint --> Resume["恢复运行时覆盖 agent.json 初始字段"]
```
...
```mermaid
flowchart TD
    Start["初始化异常或角色行为不对"] --> Name["检查角色名是否在 personas 或 --agents 中"]
    Name --> NameOK{"角色名是否有效"}
    NameOK -->|否| FixName["修正角色名或 start.py 角色清单"]
    NameOK -->|是| Config["检查 agent.json 字段是否完整"]
    Config --> ConfigOK{"是否包含 name / coord / currently / scratch / spatial"}
    ConfigOK -->|否| FixConfig["补齐角色种子配置 agent.json"]
    ConfigOK -->|是| Coord["检查初始坐标 coord 对应的地图格子 Tile"]
    Coord --> CoordOK{"是否落在有效游戏对象 game_object 地址上"}
    CoordOK -->|否| FixCoord["修正坐标或地图地址 maze.json"]
    CoordOK -->|是| Spatial["检查空间记忆 Spatial"]
    Spatial --> SpatialOK{"是否能派生睡觉地址并找到关键地点"}
    SpatialOK -->|否| FixSpatial["修正 spatial.address 或 spatial.tree"]
    SpatialOK -->|是| Runtime["检查公共运行配置 data/config.json"]
    Runtime --> RuntimeOK{"percept / schedule / think / associate 是否完整"}
    RuntimeOK -->|否| FixRuntime["修正公共运行配置"]
    RuntimeOK -->|是| Action["检查初始行动 Action 是否写入地图"]
    Action --> ActionOK{"地图格子 Tile 上是否有角色事件和对象事件"}
    ActionOK -->|否| FixAction["检查 Agent.__init__() 与 move()"]
    ActionOK -->|是| Prompt["检查基础提示词 base_desc 是否能填出角色身份"]
    Prompt --> Done["初始化链路基本成立，再进入单步仿真验证"]
```
