...
如果使用 MiniMax LLM，因为 MiniMax OpenAI 兼容接口不提供 embedding，README 建议 embedding 使用 HuggingFace 本地模型。这说明 LLM provider 和 embedding provider 可以分离。模型负责生成行为。embedding 负责记忆检索。两者不一定来自同一供应商。
...
| embedding 分离 | embedding provider 与 LLM provider 分离，支持 HuggingFace、Ollama、OpenAI。 |
