...
2. 生成式智能体 Generative Agents 当前默认如何接入 Ollama？
...
```mermaid
flowchart TD
    Model["选择中文本地模型"] --> Config["配置模型提供方 provider 与结构化输出"]
    Config --> Run["运行小规模仿真"]
    Run --> Check["检查 JSON / schema 通过率"]
    Check --> Behavior["观察日程、对话、记忆、反思"]
    Behavior --> Evidence["保存移动回放 movement / 时间线 simulation / 对话记录 conversation"]
    Evidence --> Compare["与远程强模型或基线比较"]
    Compare --> Tune["调整模型、提示词 prompt 或参数"]
    Tune --> Run
```
...
生成式智能体 Generative Agents 的模型配置文件位于：
...
这里有两个关键信息。第一，项目把“生成模型”和“嵌入模型”分开配置。当前仓库用 MiniMax 生成行为，也用 MiniMax 向量嵌入 embedding 检索记忆。生成模型负责：
...
第二，本章讨论的是中文本地模型实验，所以读者要把同一个配置入口切换成 Ollama 或其他本地模型提供方 provider。一个本地实验配置可以写成下面这样：
...
这里要注意，Ollama 的两个接口路径不完全一样。大语言模型 LLM 配置中写的是：

```text
http://127.0.0.1:11434/v1
```

向量嵌入 embedding 配置中写的是：

```text
http://127.0.0.1:11434
```

这不是笔误。项目中的 `OllamaLLMModel` 会对大语言模型 LLM 的 base_url 做处理：
...
```mermaid
flowchart TD
    Config["data/config.json"] --> LLM["生成模型 think.llm"]
    Config --> Embedding["记忆向量 associate.embedding"]
    LLM --> Provider["模型提供方 provider: minimax / ollama / openai"]
    Embedding --> Index["LlamaIndex 记忆检索"]
    Provider --> Behavior["日程、对话、反思和结构化判断"]
    Index --> Retrieval["相关记忆召回"]
    Behavior --> Experiment["本地模型实验结果"]
...
如果模型提供方 provider 是 `ollama`，会创建：

```text
OllamaLLMModel
```

如果模型提供方 provider 是 `openai`，会创建：

```text
OpenAILLMModel
```

如果模型提供方 provider 是 `minimax`，会创建：
...
第四，如果调用方传入 Pydantic return_type，就把 JSON Schema 传给 Ollama 的 format 参数。这说明生成式智能体 Generative Agents 不是简单把提示词 prompt 丢给模型。它试图让本地模型直接输出结构化结果。许多本地模型实验失败，并不是“模型不会回答”，而是：
...
生成式智能体 Generative Agents 的外在行为看起来像自然语言系统。但源码内部大量依赖结构化中间结果。例如：
...
生成式智能体 Generative Agents 已经在 `parse_structured_output()` 中做了容错：
...
短实验用来检查接口。中等实验用来检查机制。长实验才用来观察社会行为。短实验跑完以后，先读 `results/checkpoints/<name>/<name>.log`，再读断点 checkpoint 和 compressed 结果。如果日志里已经出现大量结构化输出失败，`simulation.md` 写得再顺，也只能说明项目容错帮你把流程撑过去了，不能说明模型适合长仿真。

实验推进逻辑图：

```mermaid
flowchart TD
    Short["短实验 6 仿真步 step"] --> Stable{"结构化输出是否稳定"}
    Stable -->|否| Fix["先修模型提供方 provider、提示词 prompt 或模型选择"]
    Stable -->|是| Medium["中等实验 12 仿真步 step"]
    Medium --> Mechanism{"日程、对话、记忆是否成立"}
    Mechanism -->|否| Diagnose["定位机制瓶颈"]
    Mechanism -->|是| Long["半天或更长实验"]
    Long --> Social["观察事件传播和社会行为"]
```
...
这里必须以实际 `ollama list` 输出为准。README、配置文件和本地模型仓库可能随着版本变化出现名称差异。第四，示例中的向量嵌入 embedding 模型是否已下载。
...
如果失败数持续增加，说明模型适配存在问题。结构化输出问题经常出现在以下提示词 prompt 上：
...
这是速率限制，不是提示词 prompt 写错，也不是角色行为变差。它会影响本次实验稳定性，但不能直接拿来评价模型推理能力。相反，如果没有 429，却反复出现 JSON 解析失败、输出解释性长文、地点不在候选列表里，才是模型适配本身的问题。
...
```mermaid
flowchart TD
    Observation["实验观察"] --> FailureType["归类失败类型"]
    FailureType --> Evidence["绑定日志、断点 checkpoint 或对话证据"]
    Evidence --> Cause["提出模型、提示词 prompt、检索或配置原因"]
    Cause --> Action["给出下一步调参或对照实验"]
```
...
中文本地模型的一个优势是中文对话语感更容易调整。但这不等于对话一定自然。对话自然性要看五点。第一，是否符合关系。熟人之间不应像客服对话。关系紧张的人不应突然过度亲密。第二，是否符合场景。咖啡馆、宿舍、教室、办公室里的话题密度应该不同。第三，是否推动事件。如果伊莎贝拉 Isabella Rodriguez 想传播派对信息，她的对话应该自然提到时间、地点或邀请。第四，是否有来回。不要每一句都像独立公告。第五，是否能结束。很多模型能开始聊天，但不擅长自然结束。生成式智能体 Generative Agents 通过对话轮数和判断逻辑控制结束，但模型仍可能输出拖沓内容。评价时可以摘录短样例。例如：
...
记忆流 Memory Stream 的价值不在于“存了很多东西”。而在于后续行为能想起正确东西。本地模型实验中，检索质量受两个部分影响：

- 向量嵌入 embedding 模型。
- 检索打分逻辑。

本章本地模型实验示例中，向量嵌入 embedding 使用：
...
第一，角色是否记得自己听过的事件。例如玛丽亚 Maria Lopez 听说派对后，后续是否提到派对。第二，角色是否把事件归因给正确来源。例如“伊莎贝拉 Isabella Rodriguez 告诉我派对”，而不是凭空说“大家都知道”。第三，角色是否在相关时刻想起相关事实。例如下午接近派对准备时间时，伊莎贝拉 Isabella Rodriguez 更应该想起派对任务。如果检索失败，常见表现是：
...
事件传播不是“有角色提了一句”。事件传播至少包含三层。第一层，信息从发起者进入对话。例如伊莎贝拉 Isabella Rodriguez 告诉玛丽亚 Maria Lopez：

```text
明天晚上咖啡馆有情人节派对。
```

第二层，接收者把信息写入记忆。后续能看到玛丽亚 Maria Lopez 知道这件事。第三层，接收者影响另一个角色或自己的计划。例如玛丽亚 Maria Lopez 告诉克劳斯 Klaus Mueller，或调整晚上的安排。本地模型实验要分别观察这三层。如果只发生第一层，说明对话生成能用，但记忆和计划没有跟上。如果发生第一层和第二层，但没有第三层，说明系统有记忆，但社会级联不足。如果三层都出现，才说明论文中的传播机制在本地模型下仍然成立。
...
本地模型不一定要承担所有任务。生成式智能体 Generative Agents 当前配置只有一个 think 大语言模型 LLM。但从系统设计角度看，可以把任务拆开：
...
使用 qwen3-embedding:0.6b-q8_0 作为向量嵌入 embedding 模型，运行 5 个角色、12 step、stride 10。
...
如果读者第一次做本地模型实验，建议按四步走。第一步，跑 README 默认示例。确保项目能启动、模型能响应、结果能压缩回放。第二步，跑 5 角色、6 仿真步 step 小实验。只检查链路是否通。第三步，跑 5 角色、12 仿真步 step 事件传播实验。开始观察对话、记忆和计划。第四步，换模型或换参数对比。例如比较：
...
不是重跑论文中的英语提示词 prompt。不是逐字复现论文里的输出。也不是证明模型越便宜越好。真正重跑的是这套机制：
...
| 配置入口 | 生成式智能体 Generative Agents 通过 `generative_agents/data/config.json` 切换 MiniMax、Ollama 等大语言模型 LLM 和向量嵌入 embedding。 |
