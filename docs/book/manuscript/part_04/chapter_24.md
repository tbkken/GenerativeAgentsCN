...
因为关系主要通过自然语言设定、对话和记忆形成。

中等难度是新角色。

需要新增角色配置、头像、纹理、初始坐标，并加入 `personas` 列表。

最难的是新地点。

因为地点牵涉后端 `maze.json`、前端 tilemap、碰撞、地址树和角色空间记忆。
...
新事件只需要改 currently 和可能的 daily_plan。
...
地图边界或无地址 tile 可能导致初始化 action 不合理。

如果不确定，先复用一个合理地点附近的坐标，再逐步调整。
...
如果想默认 25 人外加新角色，也要考虑 `--agent-count` 的行为。

默认列表顺序会影响仿真中 agent 思考顺序。

## 24.8 新关系写在哪里

新关系通常写在角色的 `learned`、`currently` 或 `daily_plan` 中。
...
GenerativeAgentsCN 的价值在于让关系通过相遇、对话、记忆和反思形成。
...
如果他们在咖啡馆相遇，玛丽亚可能提到讨论会。

对话后，系统可能生成新的关系 memory。
...
`maze.json` 需要新增 tile 地址、collision、sector、arena、game_object。
...
相关角色的 `spatial.tree` 要知道新地点。

第四，prompt 语义。

对象名称要让模型理解用途。

如果只改 `spatial.tree`，但 `maze.json` 没有对应地址，角色可能找不到实际 tile。

如果只改 `maze.json`，但前端地图没显示，回放会怪。

所以新增地点不是一个 JSON 字段能解决。
...
如果要做“艺术展”，可以先设在咖啡馆或公园。

如果要做“社区会议”，可以先设在公共休息室或咖啡馆。

只有当现有地点无法表达事件需求时，才考虑改地图。

## 24.13 给现有地点增加新用途

一种低风险方法是给角色设定中加入地点新用途。
...
1. 扩展优先级建议是新关系、新事件、新角色、新地点。
2. 新角色需要 `agent.json`、头像、纹理，并加入 `start.py` 的 personas。
3. `agent.json` 中最重要的是 currently、scratch 和 spatial。
4. 新关系可以写在 learned、currently 或 daily_plan 中。
5. 关系设定应提供倾向，不应硬编码结果。
6. 新地点最难，因为涉及 `maze.json`、前端 tilemap、spatial tree 和 prompt 语义。
7. 很多新事件可以先放在现有地点，不必改地图。
8. 新角色、新关系、新地点都应该先用小规模仿真验证。
9. 扩展实验必须记录具体改动，避免结果无法复现。

下一章讲如何用中文本地模型重跑论文思想。我们会把实验关注点从“改小镇内容”转向“换模型后行为质量如何变化”。
