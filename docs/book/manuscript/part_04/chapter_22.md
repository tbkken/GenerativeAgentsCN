...
这一章复现另一条论文经典社会现象：镇长竞选信息扩散。

在论文中，Sam Moore 有竞选镇长的意图。信息通过对话传播给其他居民。

GenerativeAgentsCN 中，山姆保留了这条设定。
...
仿真开始时，山姆知道自己要竞选镇长。运行后，其他角色是否知道？

第二层，扩散路径是否可追踪。

谁从山姆那里听到？谁又告诉别人？每条路径是否能在 `conversation.json` 中找到？
...
汤姆是关键反对或怀疑节点。项目设定中，汤姆对山姆并不友好，适合观察负面态度。
...
如果某个角色后来说知道山姆竞选，但没有上游对话或 memory，就不能算真实传播。
...
派对实验更依赖 planning 和到场行为。

竞选实验更依赖 dialogue、relation memory 和 reflection。

派对成功可以用到场率衡量。

竞选成功不能只用“知道人数”衡量，还要看：
...
如果只看到山姆自言自语，没有传播，说明社交触发不足。

如果所有人都无条件支持，说明角色差异不足。

如果很多人凭空知道山姆竞选，说明幻觉或共享信息污染。
...
如果一个角色从未接触山姆，也没有听别人提到，却说知道竞选，就是幻觉。
...
竞选实验中幻觉更容易发生，因为“镇长竞选”是常见语义，模型可能凭常识补全。
...
1. 山姆是竞选信息源，汤姆是关键态度测试角色。
2. 实验不仅统计知道人数，还要记录支持、怀疑、反对和观望。
3. 推荐小规模角色是山姆、汤姆、约翰、拉托亚、乔治、伊莎贝拉。
4. 运行后重点检查 `conversation.json` 和 `simulation.md`。
5. 判断知道竞选要有信息来源证据。
6. 态度分类建议使用支持、怀疑、反对、观望、未知。
7. 竞选实验比派对实验更能暴露过度礼貌和角色差异不足。
8. 幻觉知道必须单独标记，不能算信息传播。
9. 扩展实验可以加入跨天记忆、reflection 和 prompt 改造。

下一章设计自己的小镇事件。我们会从论文复现转向项目扩展，学习如何设计新事件并让它进入角色设定、日程、对话和评价链路。
