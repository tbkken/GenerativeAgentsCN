...
一个好的 GenerativeAgentsCN 事件应该能进入完整行为链：
...
这个事件适合当前项目，因为克劳斯的原始设定就是社会学学生，正在写相关论文。

本章要回答七个问题：

1. 什么样的事件适合 GenerativeAgentsCN？
2. 如何把事件写入角色设定？
3. 如何选择地点、时间和参与者？
4. 如何运行自定义事件实验？
5. 如何判断事件是否传播？
...
如果角色在图书馆但只是读书，不一定算参加。

如果角色和克劳斯在图书馆讨论社会议题，可以算参加。
...
如果只有克劳斯自己在图书馆，不能算社会事件成功。

如果多人知道但没人到场，说明传播成功但协同行动失败。

如果有人到场但没有信息来源，可能是偶然或幻觉。

## 23.11 不要硬编码结果

设计事件时，最容易犯的错是硬编码所有角色都会参加。

例如，直接把每个角色 `currently` 都写成：
...
第二步，修改发起者 daily_plan，加入准备或邀请。

第三步，选择更容易相遇的角色。

第四步，选择公共地点，例如咖啡馆或图书馆。

第五步，延长仿真时间。

第六步，微调对话 prompt，让角色更愿意提及当前计划。
...
generate_chat 没检索到当前目标。

第四，摘要丢失关键细节。

时间地点没有进入 chat memory。
...
不要只说“模型不行”，要定位是 planning、dialogue、memory 还是 spatial。
...
1. 好事件需要发起者、时间、地点、传播路径、差异反应和可观察结果。
2. 示例事件是克劳斯组织中产阶级化讨论会。
3. 最小改法是修改发起者 `currently`。
4. 角色选择要服务传播路径和评价目标。
5. 运行后重点看 `simulation.md`、`conversation.json` 和 `movement.json`。
6. 判断事件发生要同时看发起者、传播、到场和行为证据。
7. 不要硬编码所有参与者，要让信息通过对话传播。
8. 失败时要定位 planning、spatial、dialogue、memory 或 reflection。
9. 自定义事件可以扩展到诗歌会、安全会议、艺术展、学习小组等。
