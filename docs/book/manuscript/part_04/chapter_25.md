...
这个问题比“本地模型能不能运行项目”更重要。

运行项目只是工程问题。

重跑论文思想是研究问题。

本章不把本地模型当成一个便宜替代品，而把它当成一次实验变量：
...
2. GenerativeAgentsCN 当前默认如何接入 Ollama？
...
一个模型可以很会聊天，但不擅长稳定输出 JSON。

一个模型可以很会写长文本，但对时间、地点和角色约束很差。

一个模型可以在单轮问答中表现不错，但在连续几十轮仿真后出现重复、遗忘和格式漂移。
...
以后读者替换更强模型、增加工具调用、升级记忆系统时，需要一个可以比较的基线。

本章建议把实验目标写成下面这样：

```text
在 GenerativeAgentsCN 中使用中文本地模型运行一个小规模小镇仿真，
...
项目中的 `OllamaLLMModel` 会对 LLM 的 base_url 做处理：
...
本地模型实验重点关注 `OllamaLLMModel`。

它做了四件关键事情。

第一，调用 Ollama 的 `/api/chat`。
...
第四，如果调用方传入 Pydantic return_type，就把 JSON Schema 传给 Ollama 的 format 参数。

这说明 GenerativeAgentsCN 不是简单把 prompt 丢给模型。
...
Generative Agents 的外在行为看起来像自然语言系统。
...
GenerativeAgentsCN 已经在 `parse_structured_output()` 中做了容错：
...
这里必须以实际 `ollama list` 输出为准。

README、配置文件和本地模型仓库可能随着版本变化出现名称差异。

第四，embedding 模型是否已下载。
...
例如情人节派对设定在 2 月 14 日晚上，前一天运行时要给传播留出时间。
...
在 `LLMModel.completion()` 中，项目会统计每个 caller 的调用情况。
...
如果失败数持续增加，说明模型适配存在问题。

结构化输出问题经常出现在以下 prompt 上：
...
例如所有人都“吃早餐、工作、休息、社交、睡觉”。

这不是错误，但会降低社会仿真的信息密度。

## 25.13 观察三：对话自然性

中文本地模型的一个优势是中文对话语感更容易调整。
...
咖啡馆、宿舍、教室、办公室里的话题密度应该不同。

第三，是否推动事件。

如果伊莎贝拉想传播派对信息，她的对话应该自然提到时间、地点或邀请。
...
GenerativeAgentsCN 通过对话轮数和判断逻辑控制结束，但模型仍可能输出拖沓内容。
...
## 25.14 观察四：记忆检索是否抓住关键事实

Memory Stream 的价值不在于“存了很多东西”。
...
GenerativeAgentsCN 的配置中，embedding 使用：
...
例如“伊莎贝拉告诉我派对”，而不是凭空说“大家都知道”。

第三，角色是否在相关时刻想起相关事实。

例如下午接近派对准备时间时，伊莎贝拉更应该想起派对任务。
...
第三层，接收者影响另一个角色或自己的计划。

例如玛丽亚告诉克劳斯，或调整晚上的安排。

本地模型实验要分别观察这三层。

如果只发生第一层，说明对话生成能用，但记忆和计划没有跟上。

如果发生第一层和第二层，但没有第三层，说明系统有记忆，但社会级联不足。

如果三层都出现，才说明论文中的传播机制在本地模型下仍然成立。
...
GenerativeAgentsCN 当前配置只有一个 think LLM。
...
项目已经对 `<think>...</think>` 做了过滤，但仍要观察边界情况。
...
不要把小规模结论外推到 25 个智能体、多天仿真和复杂社会结构。
...
如果读者第一次做本地模型实验，建议按四步走。

第一步，跑 README 默认示例。

确保项目能启动、模型能响应、结果能压缩回放。

第二步，跑 5 角色、6 step 小实验。

只检查链路是否通。

第三步，跑 5 角色、12 step 事件传播实验。
...
如果本地中文模型能够支撑这条链，即使行为质量不如最强模型，它也说明论文思想具备可迁移性。
...
本章讲了如何用中文本地模型重跑 Generative Agents 的论文思想：

1. 本地模型实验的目标不是证明小模型等价于强模型，而是验证论文机制在中文本地环境下的可迁移性。
2. GenerativeAgentsCN 当前通过 `generative_agents/data/config.json` 配置 Ollama LLM 和 embedding。
3. 结构化输出通过率是第一道关卡，不能只看仿真是否跑完。
4. 日程、对话、记忆、反思、事件传播都要分别评价。
5. 小规模实验应先跑通 5 个角色、短时间窗口，再逐步扩大。
6. 实验记录必须包含模型、硬件、参数、角色数量、事件设定和失败样例。
7. 本地模型的常见问题包括 JSON 失败、`<think>` 泄漏、对话礼貌化、时间地点错乱、反思复述化和记忆召回噪声。
8. 成功标准应该限定在具体实验规模下，避免过度外推。
9. 本地模型实验的结果会直接指向后续前沿升级方向。
