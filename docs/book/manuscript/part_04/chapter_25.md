...
如果中途因为限流、网络或进程中断停下，可以从已有 checkpoint 继续跑。例如本章真实实验中，前半段跑到 `20240213-15:00` 后中断，后续用下面命令恢复：

```bash
python start.py --name book-election-extended --resume --step 56 --stride 10 --verbose info --log book-election-extended-resume.log
```

恢复后最终生成 96 个 checkpoint，从 `simulate-20240213-0800.json` 到 `simulate-20240213-2350.json`。
...
```text
地点：the Ville，哈维奥克供应店，供应店，供应店产品货架

乔治：山姆，你准备得怎么样了？民意调查的数据和反馈邮件分析得如何了？我们今天可以开始构建模型来优化你的竞选策略。
山姆：乔治，我准备得差不多了。民意调查的数据和反馈邮件我已经整理好了，你可以开始分析了。
乔治：我会先对这些数据进行初步分析，找出关键的趋势和模式。然后我们可以基于教育、经济政策及社区安全等议题的数据来构建数学模型，预测不同策略的效果，并据此优化你的竞选方案。
...
```text
generative_agents/results/checkpoints/book-election-extended/conversation.json
generative_agents/results/compressed/book-election-extended/simulation.md
generative_agents/results/compressed/book-election-extended/movement.json
```

先做一致性检查。`movement.json` 中的起始时间是 `2024-02-13T08:00:00`，步长 `stride` 是 `10`，参与仿真的 10 个角色是：

```text
山姆, 汤姆, 约翰, 拉托亚, 乔治, 伊莎贝拉, 阿伊莎, 亚当, 玛丽亚, 克劳斯
```

这次 `simulation.md` 中竞选相关关键词的分布如下：
