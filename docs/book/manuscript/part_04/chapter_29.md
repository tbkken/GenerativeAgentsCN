...
| 指标 | 数值 | 证据文件 | 解释 |
| --- | ---: | --- | --- |
| 时间冲突数 `schedule_conflict_count` |  |  |  |
| 目标相关行动率 `goal_related_action_rate` |  |  |  |
| 计划行动匹配率 `plan_action_match_rate` |  |  |  |
| 知情角色数 `unique_informed_agents` |  |  |  |
| 事实保持分数 `fact_preservation_score` |  |  |  |
| 到场率 `arrival_rate` |  |  |  |

## 大模型裁判 LLM as Judge 裁判摘要

| 维度 | 分数 score | 置信度 confidence | 人工复核 needs_human_review | 证据编号 evidence_ids | 裁判理由 |
| --- | ---: | ---: | --- | --- | --- |
| 计划合理性 |  |  |  |  |  |
| 社会传播可信度 |  |  |  |  |  |
| 行动落地能力 |  |  |  |  |  |
