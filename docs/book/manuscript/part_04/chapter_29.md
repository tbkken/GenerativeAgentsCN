...
```mermaid
flowchart TD
    Persona[身份一致性<br/>角色设定 agent.json / 角色画像字段 scratch] --> Memory[记忆连续性<br/>对话 conversation / 断点 checkpoint]
...
例如，一个可信的伊莎贝拉 Isabella Rodriguez 应该：
...
这不意味着伊莎贝拉 Isabella Rodriguez“真的想办派对”。它意味着系统生成的行为与角色设定、记忆和环境约束一致。本章所有评价都建立在这个边界上。可信行为是可观察行为。不是意识证明。
...
这六个维度对应论文架构。身份一致性对应身份设定 persona 和角色画像字段 scratch。记忆连续性对应记忆流 memory stream 和记忆检索 retrieval。计划合理性对应规划 planning 和日程 schedule。反应合理性对应反应 reacting。社会传播可信度对应对话 dialogue、记忆 memory 和多智能体互动 multi-agent interaction。行动落地能力对应沙盒落地 sandbox grounding、世界地图 maze 和移动回放 movement。用这六个维度，可以把“这个智能体像不像人”拆成可检查问题。
...
| `--agent` | 被评价角色 evaluated agent | 本次主要计算克劳斯 Klaus Mueller 的日程、行动和记忆。 |
...
传统关键词评估 keyword evaluation 适合算“出现了几次”，不适合判断“是否可信”。比如克劳斯 Klaus Mueller 说了“置换效应”，阿伊莎 Ayesha Khan 也说了“置换效应”，关键词脚本只能判断话题相关。它不能稳定判断下面这些更重要的问题：
...
| 行动落地能力 | 4 | 0.85 | 否 | 阿伊莎 Ayesha Khan 16:00 到达图书馆，承诺落地；但承诺主体不是克劳斯 Klaus Mueller，样本量只有 1。 |
...
大模型裁判 LLM as Judge 会进一步指出：这条到场证据的主体是阿伊莎 Ayesha Khan，而本次主要评价角色是克劳斯 Klaus Mueller。因此它给 4 分，并把“承诺主体不是克劳斯 Klaus Mueller、样本量只有 1”写进裁判理由和风险字段。不同裁判 judge 运行可能会把这个问题标成需要人工复核 `needs_human_review=true`，也可能只放在风险字段 `risks` 中；正式报告应对这类证据归属问题人工复核。这就是语义裁判比关键词统计更强的地方。它没有否定到场事实，但能识别“证据归属”这个关键问题。
...
它包括职业、年龄、家庭、兴趣、关系、习惯和当前目标。例如克劳斯 Klaus Mueller 是社会学学生，正在写关于低收入社区中产阶级化影响的研究论文。一个可信的克劳斯 Klaus Mueller 应该在以下方面保持一致：

- 日程中有学习、写作或校园活动。
- 对社会议题有较高兴趣。
- 与玛丽亚 Maria Lopez 交流时可以谈到研究。
- 不应突然自称咖啡馆老板或市长候选人。
...
这不是简单看角色能不能说“我记得”。要看它是否能引用真实经历。例如，玛丽亚 Maria Lopez 说：
...
- 如果玛丽亚 Maria Lopez 从未见过伊莎贝拉 Isabella Rodriguez，却突然说自己知道派对，应该标记为幻觉或设定泄漏。
- 如果玛丽亚 Maria Lopez 听过派对，但后续完全忘记，说明传播失败或检索失败。
- 如果玛丽亚 Maria Lopez 记得派对，但说错时间地点，说明事实保持能力不足。
...
- 伊莎贝拉 Isabella Rodriguez 在咖啡馆活动合理。
- 学生去学院、图书馆、宿舍合理。
- 角色不要频繁前往不存在或无关地点。

第三，目标合理。例如：

- 山姆 Sam Moore 竞选期间应有社区沟通行为。
- 克劳斯 Klaus Mueller 写论文期间应有研究、阅读或讨论。
- 伊莎贝拉 Isabella Rodriguez 筹备派对期间应有准备、邀请或布置。
...
| `rel(action, G)` | 含义 | 例子 |
| ---: | --- | --- |
| `1` | 直接服务目标 | 克劳斯 Klaus Mueller“撰写社区文化变迁章节”服务中产阶级化论文目标。 |
| `0.5` | 间接服务目标 | 克劳斯 Klaus Mueller“吃晚饭休息”间接支持后续写作。 |
| `0` | 无关 | 克劳斯 Klaus Mueller 计划组织讨论会，但行动是处理完全无关事务。 |
...
克劳斯 Klaus Mueller 在这个断点中的日程没有时间冲突：
...
克劳斯 Klaus Mueller 在这段时间的地点落地很稳定，7 次采样都在奥克山学院图书馆。目标相关行动率 `goal_related_action_rate` 不是满分，原因是 16:00 的“回顾笔记并补充缺失信息”没有命中目标关键词。计划行动匹配率 `plan_action_match_rate` 也不是满分，原因是 16:50 的“融入访谈引用和田野观察案例”服务论文写作，但与当前计划中的“社区文化变迁章节”缺少足够直接的短语重叠。这个结果适合作为机器初筛；人工复核时可以把 16:50 标为“间接匹配”。
...
- 伊莎贝拉 Isabella Rodriguez 遇到熟人时，可能自然提到派对。
- 山姆 Sam Moore 遇到邻居时，可能谈竞选。
- 克劳斯 Klaus Mueller 遇到玛丽亚 Maria Lopez 时，可能聊研究和兴趣。
...
这是生成式智能体 Generative Agents 最重要的魅力之一。但也是最容易被误判的地方。一个事件被多人提到，不一定说明传播成功。可能是每个人初始设定里都知道。可能是模型幻觉。可能是提示词 prompt 泄漏。可能是压缩文本让读者误以为发生了传播。可信传播至少要满足三点。第一，有源头。例如派对源头是伊莎贝拉 Isabella Rodriguez，竞选源头是山姆 Sam Moore。第二，有路径。例如：
...
这里的 `G` 是目标关键词 goal keywords，`F` 是核心事实 core facts。两者不完全一样。以派对实验为例，`G` 可以包括“筹备”“邀请”“参加”，`F` 必须包括“伊莎贝拉 Isabella Rodriguez”“下午5点”“霍布斯咖啡馆”。目标词用来判断话题相关，核心事实用来判断传播有没有变形。
...
那么传播深度是 `2`。如果伊莎贝拉 Isabella Rodriguez 只分别告诉玛丽亚 Maria Lopez 和克劳斯 Klaus Mueller，路径是：
...
| 符号 | 本次取值 | 用途 |
| --- | --- | --- |
| `S` | 克劳斯 Klaus Mueller | 传播源头 source agent。 |
...
| 指标 | 结果 | 解读 |
| --- | ---: | --- |
| 知情角色数 `unique_informed_agents` | `3` | 克劳斯 Klaus Mueller、阿伊莎 Ayesha Khan、沃尔夫冈 Wolfgang Schulz 都进入相关对话。 |
| 源头提及数 `source_mention_count` | `2` | 克劳斯 Klaus Mueller 把论文话题提给阿伊莎 Ayesha Khan 和沃尔夫冈 Wolfgang Schulz。 |
| 直接邀请数 `direct_invitation_count` | `0` | 克劳斯 Klaus Mueller 没有明确邀请别人参加活动，这次更像研究讨论，不是活动动员。 |
| 间接提及数 `indirect_mention_count` | `7` | 阿伊莎 Ayesha Khan 和沃尔夫冈 Wolfgang Schulz 多次主动延续“置换效应”等话题。 |
| 传播深度 `diffusion_depth` | `1` | 话题主要停留在克劳斯 Klaus Mueller 与两位对话对象之间，没有出现第二跳外扩。 |
...
这个结果不能写成“话题已经在小镇广泛传播”。它更准确地说明：克劳斯 Klaus Mueller 的论文主题在图书馆小圈子里形成了可追踪讨论，阿伊莎 Ayesha Khan 和沃尔夫冈 Wolfgang Schulz 会围绕“置换效应”给出反馈，但还没有扩散到更多角色。

源头提及数 `source_mention_count` 和直接邀请数 `direct_invitation_count` 必须分开。克劳斯 Klaus Mueller 问阿伊莎 Ayesha Khan“置换效应那段怎么写”是源头提及，不是邀请。阿伊莎 Ayesha Khan 说“四点图书馆老位置见”是一个后续约定，但发言者不是克劳斯 Klaus Mueller，所以在 `S = 克劳斯` 的口径下不计入直接邀请。换成派对实验时，伊莎贝拉 Isabella Rodriguez 说“明天下午 5 点来霍布斯咖啡馆参加情人节派对”，才应该计入直接邀请。
...
第 26 章的真实对话里，阿伊莎 Ayesha Khan 在 15:20 对克劳斯 Klaus Mueller 说：
...
这条证据说明阿伊莎 Ayesha Khan 的约定落地了：她在承诺时间出现在图书馆桌子附近。再结合 16:50 的对话记录，阿伊莎 Ayesha Khan 继续围绕置换效应和文本论证给克劳斯 Klaus Mueller 反馈，行动不是单纯到场，而是进入了约定主题。
...
本书建议关注三类现象。第一，信息扩散。例如派对、竞选、讨论会消息是否传播。第二，关系变化。例如玛丽亚 Maria Lopez 和克劳斯 Klaus Mueller 是否因为共同话题增加互动。第三，协同行动。例如多人是否在某个时间窗口到达同一地点参加活动。端到端评价必须配合证据链。例如“多人参加派对”需要：
...
这在传统任务型系统中合理。但在社会仿真中不完全合理。可信行为需要差异。如果所有人都无条件接受邀请，社会仿真反而不可信。如果所有人都支持山姆 Sam Moore 竞选，汤姆 Tom Moreno 的人设就没有发挥作用。如果每个角色都准时、理性、高效、合作，小镇会变成任务调度器，而不是社会仿真。因此，评价时要记录负样本：
...
每次实验结束后，可以写一个简短评价报告。问题不在于“不能用表格”，而在于不要把整份模板塞进 markdown 代码围栏里。代码块里的表格不会按书稿版式渲染，读者看到的是一大段竖线文本，反而难读。

更适合书稿展示的方式，是把报告拆成几个固定模块：字段稳定、适合横向比较的内容用表格；需要贴原文、解释原因或给下一步动作的内容用证据卡或诊断卡。这样既保留表格的扫描效率，也避免宽表挤爆版面。
...
自动指标最适合用表格，因为每一行都有相同字段：指标、数值、证据来源和解释。这里保留表格，但让每个单元格短一点，避免把长段判断塞进表格。

| 指标 | 数值 | 证据文件 | 解释 |
| --- | ---: | --- | --- |
| 时间冲突数 `schedule_conflict_count` |  |  | 日程是否存在重叠、无效时长或边界错误 |
| 目标相关行动率 `goal_related_action_rate` |  |  | 行动是否服务当前实验目标 |
| 计划行动匹配率 `plan_action_match_rate` |  |  | 实际行动是否能对应到当前计划 |
| 知情角色数 `unique_informed_agents` |  |  | 有证据知道事件或信息的角色数量 |
| 事实保持分数 `fact_preservation_score` |  |  | 传播中时间、地点、人物、事实是否保持 |
| 到场率 `arrival_rate` |  |  | 承诺到场者是否真的进入目标地点 |

表格下方再补一句解释，写机器指标不能覆盖的边界：

```text
机器指标只能做初筛。本次需要人工复核的是：……
```

### 模块四：大模型裁判 LLM as Judge 摘要

裁判结果也可以用表格，但不要把裁判理由写成长段。表格只放结构化字段，理由放到表后面的简短说明里。

| 维度 | 分数 score | 置信度 confidence | 需要人工复核 | 证据编号 |
| --- | ---: | ---: | --- | --- |
| 计划合理性 |  |  |  |  |
| 社会传播可信度 |  |  |  |  |
| 行动落地能力 |  |  |  |  |
...
关键证据先用表格建索引。原文摘录不要塞进表格，放到表格后面逐条引用。

| 编号 | 类型 | 文件 | 时间 / 节点 | 支撑的结论 |
| --- | --- | --- | --- | --- |
| E1 | 对话证据 |  |  |  |
| E2 | 记忆证据 |  |  |  |
| E3 | 位置证据 |  |  |  |
...
失败样例适合用诊断表。表格里只写短语，复杂证据回到 `E1`、`E2` 这类证据编号。

| 编号 | 现象 | 证据 | 可能原因 | 定位模块 | 下一步修改 |
| --- | --- | --- | --- | --- | --- |
| F1 |  |  |  |  |  |
| F2 |  |  |  |  |  |
| F3 |  |  |  |  |  |
