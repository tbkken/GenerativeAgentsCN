# 论文概念到 Generative Agents 源码模块映射表 v0.1

## 文档目标

本文件承接 `01_original_paper_outline.md`，用于建立“论文概念 -> Generative Agents 工程实现 -> 后续写作章节”的证据链。
...
| 论文概念 | Generative Agents 实现 | 核心讲解章节 |
|---|---|---|
| Smallville / Sandbox Environment | `frontend/static/assets/village`、`Maze`、`Tile`、Phaser 前端 | 第 12-14 章、第 23 章 |
| Agent Persona | `agent.json`、`Scratch`、`base_desc`、`currently` | 第 4、15 章 |
| Time and Simulation Step | `Timer`、`SimulateServer.simulate()`、`stride` | 第 16 章 |
| Observation | `Maze.get_scope()`、`Agent.percept()`、`Event` | 第 17 章 |
| Memory Stream | `Associate`、`Concept`、`Event`、LlamaIndex | 第 18 章 |
| Importance | `poignancy_event`、`poignancy_chat`、`Agent._add_concept()` | 第 18 章 |
| Retrieval | `AssociateRetriever`、`retrieve_focus()`、`LlamaIndex.retrieve()` | 第 18 章 |
| Reflection | `Agent.reflect()`、`reflect_focus`、`reflect_insights` | 第 21 章 |
| Planning | `Schedule`、`Agent.make_schedule()`、schedule prompts | 第 19 章 |
| Reacting | `Agent._reaction()`、`_wait_other()`、`_chat_with()` | 第 20 章 |
| Dialogue | `generate_chat`、`summarize_relation`、`summarize_chats` | 第 20 章 |
| Sandbox Grounding | `Spatial`、`Maze`、`determine_sector/arena/object` | 第 14、19 章 |
| Structured LLM Calls | `Scratch`、Pydantic response model、`LLMModel` | 第 22 章 |
| Checkpoint / Replay | `start.py`、`compress.py`、`replay.py`、`live.py` | 第 23 章 |
| Evaluation Material | `simulation.md`、`movement.json`、`conversation.json` | 第 29 章 |
...
论文强调用户可以通过自然语言干预智能体或环境。当前 Generative Agents 主要支持仿真、断点恢复、压缩和回放，没有完整实现论文中的自然语言实时干预界面。
...
Generative Agents 做了深度中文化。角色名、地点名、动作描述、prompt 都尽量使用中文，目的是降低中英混杂导致模型切换语境的风险。
...
论文以交互式沙盒展示为主。Generative Agents 更强调命令行可复现实验，因此时间推进、checkpoint 和 replay 数据生成更显式。
...
论文中的 memory stream 是理论架构。Generative Agents 通过 LlamaIndex 做工程落地，并增加了过期时间、节点清理、三类记忆列表等实现细节。
...
## 12. Sandbox Grounding / Spatial Memory
...
这些是 Generative Agents 很值得写的工程特色。
...
## 15. Structured Prompt and LLM Adapter
...
论文时期主要依赖 GPT-3.5 / GPT-4 API。Generative Agents 面向中文本地模型和 OpenAI-compatible API 做了更多适配。
...
## 16. Checkpoint, Compression and Replay
...
论文展示的是 Smallville 系统和实验结果。Generative Agents 增加了更直接的 Markdown 时间线输出，适合教学、写作和实验复盘。
...
| 后续章节 | 主要论文概念 | 主要代码锚点 |
|---|---|---|
| 第 12 章 项目谱系 | 原始系统、重构、中文化 | README、README_en、项目目录 |
| 第 13 章 概念映射 | 全部核心概念 | 本文件 |
| 第 14 章 世界模型 | Smallville、Sandbox Grounding | `maze.py`、`maze.json`、tilemap |
| 第 15 章 智能体初始化 | Persona、Memory、Schedule | `Agent.__init__()`、`agent.json` |
| 第 16 章 仿真循环 | Agent Runtime Loop | `start.py`、`game.py`、`Agent.think()` |
| 第 17 章 感知 | Observation | `Agent.percept()`、`Maze.get_scope()` |
| 第 18 章 记忆 | Memory Stream、Retrieval、Importance | `associate.py`、`storage/index.py` |
| 第 19 章 日程 | Planning | `schedule.py`、`make_schedule()` |
| 第 20 章 社交 | Reaction、Dialogue | `_reaction()`、`_chat_with()` |
| 第 21 章 反思 | Reflection | `reflect()`、reflect prompts |
| 第 22 章 模型适配 | LLM Calls、Structured Output | `scratch.py`、`llm_model.py` |
| 第 23 章 回放系统 | Observation for Evaluation | `compress.py`、`replay.py`、`live.py` |
| 第 23-24 章 实验复现 | Information Diffusion | `conversation.json`、`simulation.md` |
| 第 29 章 可信评价 | Evaluation、Ablation | checkpoint、统计脚本、实验设计 |
