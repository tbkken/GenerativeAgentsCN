...
4. Prompt 文件必须当作源码讲。
   - Generative Agents 的行为很大一部分由 `data/prompts/*.txt` 决定。
5. 示例数据必须进入讲解。
...
14. 世界模型：地图、Tile、地址树、空间记忆
15. 智能体初始化：角色设定如何进入系统
16. 仿真循环：`start.py`、`Game`、`Agent.think()`
17. 感知：智能体如何看见附近事件
18. 记忆：事件、对话、想法如何存储和检索
19. 日程：从一天计划到具体动作
20. 社交：聊天、等待、关系传播
21. 反思：重要事件如何变成长期认知
22. 模型适配：Ollama、MiniMax、OpenAI、结构化输出
23. 回放系统：checkpoint、`movement.json`、`simulation.md`、Phaser 前端

---

## 第 14 章：世界模型：地图、Tile、地址树、空间记忆
...
## 第 16 章：仿真循环：`start.py`、`Game`、`Agent.think()`
...
完整讲清 Generative Agents 的长期记忆系统。
...
## 第 22 章：模型适配：Ollama、MiniMax、OpenAI、结构化输出
...
讲清 Generative Agents 如何让不同大模型稳定驱动智能体。
...
## 第 23 章：回放系统：checkpoint、`movement.json`、`simulation.md`、Phaser 前端
