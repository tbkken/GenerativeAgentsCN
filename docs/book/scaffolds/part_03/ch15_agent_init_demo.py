...
def main() -> int:
    ...
    print("第 15 章智能体初始化脚手架 agent-initialization scaffold")
    print("=" * 46)
    print(f"角色配置 agent_config: {agent_path.relative_to(ROOT)}")
    print(f"运行配置 runtime_config: {config_path.relative_to(ROOT)}")
    print()
    print("已加载身份 loaded identity")
    print(f"角色名 name: {agent.name}")
    print(f"初始坐标 coord: {agent.coord}")
    print(f"地图地址 tile_address: {tile.get_address(as_list=False)}")
    print(f"当前关注 currently: {agent.scratch.currently}")
    print()
    print("人格草稿字段 Scratch fields")
    for key in ["age", "innate", "learned", "lifestyle", "daily_plan"]:
        print(f"{key}: {agent.scratch.config[key]}")
    print()
    print("空间记忆 Spatial memory")
    print(f"居住地址 living_area: {' -> '.join(agent.spatial.address['living_area'])}")
    print(f"睡觉地址 sleep_address: {' -> '.join(agent.spatial.address['睡觉'])}")
    print(f"已知世界根节点 known_world_roots: {', '.join(agent.spatial.tree.keys())}")
    print()
    print("运行模块 Runtime modules")
    print(f"感知配置 percept_config: {agent.percept_config}")
    print(f"思考模型提供方 think_provider: {agent.think_config['llm']['provider']}")
    print(f"关联记忆节点数 associate_nodes: {agent.associate.index.nodes_num}")
    print(f"日程创建时间 schedule_created: {agent.schedule.create}")
    print(f"日程条目数 schedule_items: {len(agent.schedule.daily_schedule)}")
    print()
    print("初始行动 Initial action")
    print(f"角色事件 event: {action['event']}")
    print(f"对象事件 object: {action.get('object')}")
    print()
    print("基础提示词预览 Base prompt preview")
