...
def _runtime_model_config(document: dict, purpose: str) -> tuple[dict, str]:
    ...
    # The legacy in-memory config model still has a Studio-era required field.
    # It is never resolved or persisted by Runtime; credentials come only from
    # the execution environment.
    if config.get("provider") == "openai":
        config["secret_ref"] = "runtime-environment"
