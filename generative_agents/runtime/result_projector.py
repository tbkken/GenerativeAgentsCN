"""A small durable projector used before the full SQLite result projection is attached."""
...
class FileResultProjector:
    """Atomically persist the visible result boundary and deterministic counters."""
        ...
        *,
        ...
        try:
            with temporary.open("x", encoding="utf-8", newline="\n") as file_handle:
                json.dump(document, file_handle, ensure_ascii=False, sort_keys=True, separators=(",", ":"))
                file_handle.flush()
    ...
    def read(self) -> dict:
        if not self._path.exists():
            return {"run_id": str(self._paths.run_id), "available_step": 0, "result_version": 0, "steps": {}}
...
# The production SQLite projector is separate so this small compatibility
# projector can still support filesystem-only legacy tests.
from .sqlite_result_projector import (  # noqa: E402
    ResultProjectionError,
    SqliteResultProjector,
)

__all__ = ["FileResultProjector", "ResultProjectionError", "SqliteResultProjector"]
