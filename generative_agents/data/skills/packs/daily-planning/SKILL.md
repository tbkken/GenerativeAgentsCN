...
先判断是创建、细化还是修订日程。把角色状态、虚拟时间、步长、Run 总步数和已有计划完整交给最匹配的 Skill。`total_steps` 表示整个 Run 的仿真步数，可能覆盖一天以上，不等于单日日程的槽位数；日程时间窗和槽位数量必须用实际分钟数核对。后续 Skill 直接读取上一个 Skill 的自然语言结果，不创建端口映射或业务 JSON 合同。
