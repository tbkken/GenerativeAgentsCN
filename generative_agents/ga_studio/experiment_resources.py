...
from generative_agents.ga_protocol import (
    PackageError, atomic_write_bytes, open_package, read_json,
    validate_experiment_directory, validate_experiment_integrity, write_integrity_manifest,
)
...
from .builder import build_semantic_index
from .workspace import ExperimentWorkspaceService, WorkspaceConflictError
from .skill_trial import SkillTrialError, run_copied_skill_trial
    ...
    def package(self, experiment_id, *, write=False, expected=None, validate_execution=False):
        with package_lock(self.workspace.package_root / "experiments" / f"{experiment_id}.identity"):
            ...
            if write and not location.is_dir():
                raise HTTPException(409, "实验已封存；请复制为新的独立实验后修改")
    ...
    def read(self, experiment_id, resource, query, *, validate_execution=False):
        with self.package(experiment_id, validate_execution=validate_execution) as (root, ep, digest, editable):
            ...
            if resource == "map-editor/ville-document":
                return world.get("definition", {}).get("editor_v2") or {}
            if family == "skills":
                ...
                if key:
                    ...
                    if entry is None:
                        raise HTTPException(404, "实验中没有这个技能")
            ...
            raise HTTPException(404, "实验资源入口不存在")

    @staticmethod
    def missing():
        raise HTTPException(404, "实验中没有这个资源")

    def trial(self, experiment_id, key, body):
        ...
        return run_copied_skill_trial(snapshots, key, body["input_text"], body.get("context", {}),
                                     config=config, base_url=config["base_url"], model=model, api_key=api_key)
        ...
        with self.package(experiment_id, write=True, expected=expected) as (root, ep, digest, editable):
            ...
            if family == "maps" and key and method in ("PUT", "POST"):
                ...
                if method == "POST" and parts[2:] != ["validate"]:
                    raise HTTPException(405, "不支持此地图操作")
                ...
                raise HTTPException(405, "此实验资源不支持该操作")
    ...
    def write_resource(experiment_id: str, resource_path: str, request: Request, body: dict):
        try:
            ...
            if request.method == "POST" and len(parts) == 3 and parts[0] == "skills" and parts[2] == "run":
                return editor.trial(experiment_id, parts[1], body)
