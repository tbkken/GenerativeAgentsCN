"""Four-layer world geometry, rendering recipes and package-local materials."""
...
from generative_agents.ga_protocol.schemas.experiment import StrictModel
from generative_agents.ga_protocol.schemas.object_skills import GameObjectSkillBinding
from generative_agents.ga_protocol.schemas.object_skills import validate_unique_skill_bindings
...
class MaterialSource(StrictModel):
    ...
    def validate_origin(self) -> "MaterialSource":
        ...
        if self.kind == "CANVAS" and any(
            (self.bundled_path, self.generated_color)
        ):
...
class WorldDocument(StrictModel):
    """实验自有的地图几何、空间层级与渲染内容。"""
    ...
    def unique_ids(cls, values: list[Any]) -> list[Any]:
        """执行 `WorldDocument` 的`unique``ids`操作。
    ...
    @model_validator(mode="after")
    def validate_relations(self) -> "WorldDocument":
        """校验`relations`。

        返回:
            返回 `'WorldDocument'` 类型的处理结果。
        ...
        if self.navigation:
            width = roots[0].bounds.width
            height = roots[0].bounds.height
