...
from generative_agents.ga_runtime.storage.logging import split_line
from generative_agents.ga_runtime.memory.action import Action
from generative_agents.ga_runtime.memory.event import Event
from generative_agents.ga_runtime.engine.actor import ActorState
...
class Game:
    ...
    def __init__(
        ...
        *,
        ...
        self.agents: dict[str, ActorState] = {}
        agents_by_name: dict[str, ActorState] = {}
        ...
        for agent_key, definition in agents.items():
            ...
            agent_config = copy.deepcopy(definition)
        ...
        # 运行时与持久化层使用不可变 agent_key；Event.subject 使用展示名。
        # 两套命名空间必须显式分离，不能把别名混入同一字典，否则迭代和快照会重复智能体。
        self.agents_by_name: Mapping[str, ActorState] = MappingProxyType(agents_by_name)
    ...
    def get_agent(self, agent_key: str) -> ActorState:
        """获取智能体。
    ...
    def commit_world_action(
        ...
        *,
        ...
        agent.profile.currently = current_status
